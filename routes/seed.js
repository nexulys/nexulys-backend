...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    await Invoice.create([
      ...
      { company: company._id, numero: 'FAC-2025-003', client: { nom: 'Global Finance' }, lignes: [{ description: 'Audit système', quantite: 3, prixUnitaire: 2000, montantHT: 6000 }], montantHT: 6000, tauxTVA: 20, montantTVA: 1200, montantTTC: 7200, statut: 'en_retard', dateEcheance: new Date(Date.now() - 15*86400000), createdBy: admin._id }
    ]);
    ...
    const supplier = await Supplier.create({ company: company._id, nom: 'TechSupply Pro', email: 'commandes@techsupply.fr', delaiLivraison: 5 });
    ...
    const project = await Project.create({ company: company._id, nom: 'Refonte Site Web', statut: 'en_cours', createdBy: admin._id });

    await Task.create([
      { company: company._id, titre: 'Maquettes UI/UX', priorite: 'haute', statut: 'termine', projet: project._id, assignee: admin._id, createdBy: admin._id },
    ...
    res.json({
      ...
      credentials: { email: 'admin@techcorp.fr', password: 'novexa2025' }
    });
  } catch (err) {
